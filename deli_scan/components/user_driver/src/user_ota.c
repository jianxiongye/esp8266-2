...
static bool connect_to_http_server()
{
    ...
    NVS_Read(IP_KEY, buf, sizeof (buf));
    0 == strlen(buf) ? sprintf(ip, "%s", EXAMPLE_SERVER_IP) : sprintf(ip, "%s", buf);
...
while (1)
{
    ...
    const esp_partition_t *configured = esp_ota_get_running_partition();
    ...
    NVS_Read(FILE_KEY, buf, sizeof (buf));
    0 == strlen(buf) ? sprintf(file, "%s", EXAMPLE_FILENAME) : sprintf(file, "%s", buf);
    int get_len = asprintf(&http_request, GET_FORMAT, file, ip, EXAMPLE_SERVER_PORT);
    if (get_len < 0) {
    ...
    printf("updata success. Prepare to restart system!\r\n");
    esp_restart();
task_fatal_error:
...
{
    xTaskCreate(&ota_example_task, "ota_example_task", 3072, NULL, 7, &ota_handle);
}
