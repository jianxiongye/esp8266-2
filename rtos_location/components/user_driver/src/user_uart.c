                ...
                case UART_DATA:
                    ...
                    if (0 == strncmp(dtmp, "set", 3))
                    {
                        switch (dtmp[3])
                        {
                            case 'N': // wifi 名字
                                ...
                                printf("uart set wifi name: %s\r\n", buf);
                                NVS_Write(WIFI_NAME_KEY, buf, sizeof (buf));
                                memset(buf, 0, sizeof (buf));
                                break;
                            case 'P': // wifi 密码
                                memcpy(buf, dtmp + 5, event.size - 5);
                                printf("uart set wifi passwd: %s\r\n", buf);
                            ...
                            case 'S': // 服务器ip + port
                                if ('1' == dtmp[5] && ':' == dtmp[6])
                                {
                                    ...
                                    {
                                        memset(buf, 0, sizeof (buf));
                                        memcpy(buf, dtmp + 7, strlen(dtmp) - 7);
                                        NVS_Write(IP_KEY, buf, sizeof (buf));
                                        ESP_LOGI(TAG, "server IP: %s", buf);
                                    }
                                    else
                                    {
                                        memset(buf, 0, sizeof (buf));
                                        memcpy(buf, dtmp + 7, (ret - dtmp) - 7);
                                        NVS_Write(IP_KEY, buf, sizeof (buf));
                                        ESP_LOGI(TAG, "server IP: %s", buf);
                                        memset(buf, 0, sizeof (buf));
                                        strcpy(buf, dtmp + (ret - dtmp) + 1);
                                        if (0 != strlen(buf))
                                        {
                                            NVS_Write(FILE_KEY, buf, sizeof (buf));
                                            ESP_LOGI(TAG, "file name: %s", buf);
                                        }
                                    }
                    ...
                    else
                    {
                        ...
                        if (HYTERA_HEADER_ID == replayData->hy_header.hy_headerId)
                        {
                            ...
                            {
                                case HYTERA_ACCEPT: 
                                    ESP_LOGI(TAG, "--0x%x----0x%x--", replayData->hy_header.hy_length, replayData->hy_header.hy_checkSum);
                                    
                                    if (data_check(dtmp, BigtoLittle16(replayData->hy_header.hy_length), replayData->hy_header.hy_checkSum))
                                    {
                                        is_connect_hytera_flag = false;
                                        xEventGroupSetBits(wifi_event_group, HYTERA_ACCEPT_BIT);
                                        ESP_LOGI(TAG, "data check ok");
                                    }
                                    break;
                                case HYTERA_REJECT: 
                                    if (data_check(dtmp, BigtoLittle16(replayData->hy_header.hy_length), replayData->hy_header.hy_checkSum))
                                ...
                                case HYTERA_DATA: 
                                    ...
                                    send_data_ack_hytera(BigtoLittle16(replayData->hy_header.hy_pn));
                                    if (REPLY_CMD == replayData->cmd && SUCCESS == replayData->result)
                                    {
                                        //printf("data ok, 0x%04x\r\n", BigtoLittle16(replayData->hy_header.hy_pn));
                                        gHy_pn = BigtoLittle16(replayData->hy_header.hy_pn) + 1;
                                        xEventGroupSetBits(wifi_event_group, HYTERA_DATA_OK_BIT);
                                    }
                                    else if (REPLY_CMD == replayData->cmd && FAILURE == replayData->result)
                                    {
